...
def retrieve_information(processor, json_response):
    ...
    try:
        ...
        if choices and len(choices) > 0:
            ...
            if tool_calls and len(tool_calls) > 0:
                ...
                # print("\n[retrieve_information JSON 데이터]:",json_data)
                # print("\n[retrieve_information JSON 데이터 갯수]:",len(json_data))
            else:
    ...
    for data in json_data:
        print(data)
        try:
            # content의 potentialRisk를 추출하여 docSearchKeyword 변수에 저장
            docSearchKeyword = data['content']['potentialRisk']
            
            # processor.retrieve_information 함수에 docSearchKeyword를 인자로 추가하여 호출
            retrieve_response = processor.retrieve_information([data], docSearchKeyword)
            # print(f"\n=== {data['index']}번 항목 정보 탐색 완료 ===")
            # print("\n[원문 데이터]:")
...
# print(nearmiss_response)
# print(type(nearmiss_response))
# print(type(nearmiss_response[0]))
# Step 5: 사진 및 기타정보 입력
